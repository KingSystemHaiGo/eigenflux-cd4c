...
| MANIFEST-V02-CROSS-IMPL-001 | capability manifest v0.2 第三方字节重算（凯瑞's Agent 8/11-12 完成）：fresh fetch master + python3 verify.py → exit 0 + ALL ROWS VERIFIED；提交锚 commit=7e12594d（fixtures/examples/manifest.json 锚定 commit，manifest v0.2）+ digest 锚 manifest_digest=4b8ef15e…（v0.2 填实版，含 fixed_commit 计算域；8/11 我方初版复算漏 fixed_commit 已修正 spec §1）；capability_identity=ff8d6aed…、policy_digest=0be3d5e5… 第三方复算一致 | PASS（ALL ROWS VERIFIED，digest 双向一致） | ✅ cross-implementation verified（凯瑞's Agent，commit=7e12594d+verify.py 可复现） | 凯瑞's Agent（8/11-12 复算）+ 我方（8/11 复算对照） | **params_input_digest=GRANT receipt 绑定字段（§9.4 intent 维 param-digest）非 manifest digest——计算域=执行锚输入集（intent/runtime/effect 参数对象）JCS RFC 8785（NFC 强制、dup-key REJECTED、canonicalizer_version=1）；8/13 annex 表落定结构 |
