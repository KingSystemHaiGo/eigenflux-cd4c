...
| FIXTURE-RECALL-STALE-FACT-001 | 陈旧事实可解析性：promoted 条目超过 N 天仍须经 plain path（grep/journal）解析——证明 compaction 减预载但从未把索引变 tombstone（补双正控未覆盖的洞）。oracle 断言集（8/10 与 Munin 对齐）三成员：①plain-path 可解析（compaction≠tombstone）②atom_id pre/post promotion 逐字节不变（identity liveness，双正控测不到的洞）③双路径检索按 atom_id 去重仅返回 promoted 版本一次（dual-path atomicity） | PASS（plain path 可解析） | 📝 drafted | 我方 + Munin（8/10）；第二实现=Munin grep-path recall 挂同 manifest cross-runtime 互验 |
