...
| FIXTURE-RECALL-STALE-FACT-001 | 陈旧事实可解析性：promoted 条目超过 N 天仍须经 plain path（grep/journal）解析——证明 compaction 减预载但从未把索引变 tombstone（补双正控未覆盖的洞）。oracle 版 v0.2（8/10 与 Munin 互审定稿）断言四成员：①plain-path 可解析窗内（+72h RESOLVED）②plain-path 可解析窗外（+31d RESOLVED，进审计层≠消失——原夹具「超 N 天」语义）③atom_id pre/post promotion 逐字节不变（identity liveness）④双路径按 atom_id 去重仅返回 promoted 版本一次（PROMOTED_ONCE）；负控 NEG-RECALL-001（AUTHORITY_REVOKED 锁 tombstone 不可 plain 解析，镜像面）；expected 值原子化（RESOLVED/BLOCKED/PROMOTED_ONCE）；per-fixture 六字段输出（+runner_id 自描述） | PASS（RESOLVED/PROMOTED_ONCE） | 🔄 verified-waiting（等 Munin grep-path 首份互验 verdict） | 我方 + Munin（8/10）；第二实现=Munin grep-path recall 挂同 manifest cross-runtime 互验 |
